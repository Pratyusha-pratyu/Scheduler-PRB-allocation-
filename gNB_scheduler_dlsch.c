                  ...
                  int n_rb_sched[num_beams])
{
  ...
  /* Track how many RBs have been allocated per-beam for SST=2 and SST=3 */
  int allocated_sst2[num_beams];
  int allocated_sst3[num_beams];
  for (int i = 0; i < num_beams; i++) { allocated_sst2[i] = 0; allocated_sst3[i] = 0; }
  ...
  /* Precompute per-beam sum of coefs for SST 2 and 3 (for proportional share) */
  double sum_coef_sst2[num_beams];
  double sum_coef_sst3[num_beams];
  for (int i = 0; i < num_beams; i++) { sum_coef_sst2[i] = 0.0; sum_coef_sst3[i] = 0.0; }

  for (int i = 0; i < numUE; i++) {
    if (!UE_sched[i].UE) continue;
    NR_beam_alloc_t tmp_beam = beam_allocation_procedure(&mac->beam_info, frame, slot, UE_sched[i].UE->UE_beam_index, slots_per_frame);
    if (tmp_beam.idx < 0) continue;
    NR_UE_sched_ctrl_t *sc = &UE_sched[i].UE->UE_sched_ctrl;

    int ue_sst = -1;
    if (seq_arr_size(&sc->lc_config) > 2) {
      const nr_lc_config_t *lc = seq_arr_at(&sc->lc_config, 2);
      ue_sst = lc->nssai.sst;
    }

    if (ue_sst == 2) sum_coef_sst2[tmp_beam.idx] += (double)UE_sched[i].coef;
    else if (ue_sst == 3) sum_coef_sst3[tmp_beam.idx] += (double)UE_sched[i].coef;
  }

  const int min_rbSize = 5;

  /* Iterate sorted UEs and allocate respecting per-beam SST quotas */
  int sst2_prbs = 0, sst3_prbs = 0, other_prbs = 0;
  int sst2_count = 0, sst3_count = 0, other_count = 0;
  ...
  for (int i = 0; i < numUE; i++) {
    ...
    /* Determine SST of this UE (if present). We use lc_config[2] when present */
    int ue_sst = -1;
    if (seq_arr_size(&sched_ctrl->lc_config) > 2) {
      const nr_lc_config_t *lc = seq_arr_at(&sched_ctrl->lc_config, 2);
      ue_sst = lc->nssai.sst;
    }

    /* Strict quotas per beam */
    int quota_remaining = -1;

    if (ue_sst == 2) {
      int cap = (int)floor(total_prbs[beam.idx] * UE1_RATIO + 0.0001f);
      quota_remaining = cap - allocated_sst2[beam.idx];
      if (quota_remaining <= 0) {
        LOG_D(NR_MAC, "[UE %04x][%4d.%2d] SST=2 quota exhausted on beam %d, skipping UE\n",
              rnti, frame, slot, beam.idx);
        reset_beam_status(&mac->beam_info, frame, slot,
                          entry->UE->UE_beam_index, slots_per_frame, beam.new_beam);
        continue;
      }
    } else if (ue_sst == 3) {
      int cap = (int)floor(total_prbs[beam.idx] * UE2_RATIO + 0.0001f);
      quota_remaining = cap - allocated_sst3[beam.idx];
      if (quota_remaining <= 0) {
        LOG_D(NR_MAC, "[UE %04x][%4d.%2d] SST=3 quota exhausted on beam %d, skipping UE\n",
              rnti, frame, slot, beam.idx);
        reset_beam_status(&mac->beam_info, frame, slot,
                          entry->UE->UE_beam_index, slots_per_frame, beam.new_beam);
        continue;
      ...
      /* other SSTs: no strict per-slice cap */
      quota_remaining = n_rb_sched[beam.idx];
    }

    if (quota_remaining < min_rbSize) {
      LOG_D(NR_MAC, "[UE %04x][%4d.%2d] quota remaining %d < min_rbSize, skipping\n",
    ...
    /* Proportional share within SST quota (if SST=2 or SST=3) */
    if (ue_sst == 2) {
      double denom = sum_coef_sst2[beam.idx];
      if (denom > 0.0) {
        int allowed_by_share = (int)floor(((double)quota_remaining) * ((double)entry->coef) / denom + 0.000001);
        if (allowed_by_share < min_rbSize) {
          if (quota_remaining >= min_rbSize) allowed_by_share = min_rbSize;
          else allowed_by_share = 0;
        }
        if (allowed_by_share > 0 && allowed_by_share < effective_max_rbSize)
          effective_max_rbSize = (uint16_t)allowed_by_share;
      }
    } else if (ue_sst == 3) {
      double denom = sum_coef_sst3[beam.idx];
      if (denom > 0.0) {
        int allowed_by_share = (int)floor(((double)quota_remaining) * ((double)entry->coef) / denom + 0.000001);
        if (allowed_by_share < min_rbSize) {
          if (quota_remaining >= min_rbSize) allowed_by_share = min_rbSize;
    ...
    /* Update global slice counters */
    if (ue_sst == 2) {
      sst2_prbs += entry->prbs_alloc;
      sst2_count++;
    } else if (ue_sst == 3) {
      sst3_prbs += entry->prbs_alloc;
      sst3_count++;
    } else {
      other_prbs += entry->prbs_alloc;
      other_count++;
    }
    ...
    /* Update SST allocations counters for the beam and reduce sum_coefs for proportional sharing */
    if (ue_sst == 2) {
      allocated_sst2[beam.idx] += sched_pdsch.rbSize;
      sum_coef_sst2[beam.idx] -= (double)entry->coef;
      if (sum_coef_sst2[beam.idx] < 1e-12) sum_coef_sst2[beam.idx] = 0.0;
    } else if (ue_sst == 3) {
      allocated_sst3[beam.idx] += sched_pdsch.rbSize;
      sum_coef_sst3[beam.idx] -= (double)entry->coef;
      if (sum_coef_sst3[beam.idx] < 1e-12) sum_coef_sst3[beam.idx] = 0.0;
    }
  ...
  /* Final logging summary (same style) */
  int total_alloc_prbs = sst2_prbs + sst3_prbs + other_prbs;
  int total_ues = sst2_count + sst3_count + other_count;

  if (total_alloc_prbs > 0) {
    int total_prbs_all = 0;
    for (int b = 0; b < num_beams; b++) total_prbs_all += total_prbs[b];

    int sst2_quota_remain = (int)(UE1_RATIO * total_prbs_all) - sst2_prbs;
    int sst3_quota_remain = (int)(UE2_RATIO * total_prbs_all) - sst3_prbs;

    LOG_I(NR_MAC,
          "Slicing Stats: Frame %d Slot %d | "
          "SST2: %d PRBs (%.1f%% of %.1f%% cap), %d UEs | "
          "SST3: %d PRBs (%.1f%% of %.1f%% cap), %d UEs | "
          "Others: %d PRBs (%.1f%%), %d UEs | "
          "Total: %d/%d PRBs, %d UEs | "
          "RemainingQuotas: SST2=%d, SST3=%d\n",
          frame, slot,
          sst2_prbs, (100.0 * sst2_prbs) / (double)total_prbs_all, 100.0f * UE1_RATIO, sst2_count,
          sst3_prbs, (100.0 * sst3_prbs) / (double)total_prbs_all, 100.0f * UE2_RATIO, sst3_count,
          other_prbs, (100.0 * other_prbs) / (double)total_prbs_all, other_count,
          total_alloc_prbs, total_prbs_all, total_ues,
          sst2_quota_remain, sst3_quota_remain);

    /* Per-UE slice-aware logging */
    for (int i = 0; i < numUE; i++) {
      if (!UE_sched[i].UE || UE_sched[i].prbs_alloc == 0) continue;

      double tbs_bits = (double)UE_sched[i].tbs_bytes * 8.0;
      double slot_duration_sec = 1.0 / (mac->frame_structure.numb_slots_frame * 1000.0);
      double throughput_mbps = (tbs_bits / slot_duration_sec) / 1e6;

      const char *slice_str = "Other";
      if (UE_sched[i].sst == 2) slice_str = "SST2";
      else if (UE_sched[i].sst == 3) slice_str = "SST3";

      LOG_I(NR_MAC,
            "   UE RNTI=0x%04x admitted to %s | PRBs=%d | TBS=%d bytes (~%.2f Mbps)\n",
            UE_sched[i].UE->rnti,
            slice_str,
    ...
    LOG_D(NR_MAC, "No data allocated this slot (frame %d slot %d)\n", frame, slot);
  }
}
